...
export default class Player extends Entity {
	...
	moveLeft(isRunning = false) {
		if(isRunning){
			this.velocityLimit = new Vector(Player.VELOCITY_LIMIT+100, Player.VELOCITY_LIMIT+100);
			this.direction = Direction.Left;
			this.velocity.x = Math.max(this.velocity.x - (this.speedScalar+10) * this.frictionScalar, -this.velocityLimit.x);
		}else{
	...
	moveRight(isRunning = false) {
		if(isRunning){
			...
			this.velocity.x = Math.min(this.velocity.x + (this.speedScalar+10) * this.frictionScalar, this.velocityLimit.x);	
		}
		else{
			this.velocityLimit = new Vector(Player.VELOCITY_LIMIT, Player.VELOCITY_LIMIT);
			this.direction = Direction.Right;
	...
	onEntityCollision(entity) {
		//console.log(this);
		if(this.bigger === true && !entity.isDead){
			this.bigger = false;
			entity.isDead = true;
			this.sprites = Player.generateSprites();
