...
import Shrapnel from "../entities/Shrapnel.js";
...
export default class Slingshot {
	...
	update(dt) {
		this.bird.update(dt);
		this.shrapnelArray.forEach((shrapnel) =>{
			shrapnel.update(dt);
			//matter.Body.applyForce(shrapnel.body, shrapnel.body.position, {x: 0.18, y: -0.02});
			//shrapnel.density = 0.08;
		});

		if(this.bird instanceof BlueBird && this.bird.split && this.blueBirdMade == false){
			this.rightBird = new BlueBird(this.bird.body.position.x, this.bird.body.position.y);
			//this.rightBird.body = GameEntity.clone(this.bird.body);
			//console.log("HERE");
			//console.log(this.rightBird.body);
			this.rightBird.body.velocity = this.bird.body.velocity;
			this.rightBird.body.speed = this.bird.body.speed;
			//console.log(this.rightBird.body);
			//console.log(this.bird.body);
            matter.Body.applyForce(this.rightBird.body, this.rightBird.body.position, {x: 0.18, y: -0.02});


            this.leftBird = new BlueBird(this.bird.body.position.x, this.bird.body.position.y);
			//this.leftBird.body = GameEntity.clone(this.bird.body);
			this.leftBird.body.speed = this.bird.body.speed;
			this.leftBird.body.velocity = this.bird.body.velocity;
            matter.Body.applyForce(this.leftBird.body, this.leftBird.body.position, {x: 0.1, y: -0.02});

			this.blueBirdMade = true;
		}

		if(this.bird instanceof BlackBird && this.bird.didStop() && !this.bird.didGoOffScreen() && this.bird.explode == false && this.bird.isLaunched){
			console.log("EXPLODE NOWWWWW");
			this.bird.explode = true;
	...
	render() {
		...
		this.bird.render();
		if(this.bird instanceof BlueBird && this.blueBirdMade){
			this.rightBird.render();
			this.leftBird.render();
		}
		else{
			this.blueBirdMade = false;
			this.rightBird = null;
	...
	launch() {
		...
		if(this.bird instanceof BlueBird)
		{
			this.bird.isLaunched = true;
		}
		if(this.bird instanceof BlackBird)
		{
			this.bird.isLaunched = true;
		}
	...
	shouldUnload() {
		...
		if(this.rightBird != null && this.leftBird != null){
			//console.log("BLUE BIRD REMOVING");
			
			return this.wasLaunched && (this.bird.didStop() || this.bird.didGoOffScreen()) && (this.rightBird.didStop() || this.rightBird.didGoOffScreen()) && (this.leftBird.didStop() || this.leftBird.didGoOffScreen());
		}
		else if(this.bird instanceof BlackBird && this.bird.exploded){
			console.log("BLACK BIRD REMOVING");
			this.bird.exploded = false;
			//console.log("exploded now so do 6 little circles of damage before leaving");
			this.shrapnelArray.push(new Shrapnel(this.bird.body.position.x, this.bird.body.position.y));
			this.shrapnelArray.push(new Shrapnel(this.bird.body.position.x-50, this.bird.body.position.y));
			this.shrapnelArray.push(new Shrapnel(this.bird.body.position.x-50, this.bird.body.position.y-50));
			this.shrapnelArray.push(new Shrapnel(this.bird.body.position.x, this.bird.body.position.y-50));
			this.shrapnelArray.push(new Shrapnel(this.bird.body.position.x+50, this.bird.body.position.y-50));
			this.shrapnelArray.push(new Shrapnel(this.bird.body.position.x+50, this.bird.body.position.y));
			// this.shrapnelArray.forEach((shrapnel)=>{
			// 	shrapnel.density = 0.01
			// });
			matter.Body.applyForce(this.shrapnelArray[0].body, this.shrapnelArray[0].body.position, {x: 0, y: -0.5});
			matter.Body.applyForce(this.shrapnelArray[1].body, this.shrapnelArray[1].body.position, {x: -0.5, y: 0});
			matter.Body.applyForce(this.shrapnelArray[2].body, this.shrapnelArray[2].body.position, {x: -0.5, y: -0.5});
			matter.Body.applyForce(this.shrapnelArray[3].body, this.shrapnelArray[3].body.position, {x: 0, y: -0.5});
			matter.Body.applyForce(this.shrapnelArray[4].body, this.shrapnelArray[4].body.position, {x: 0.5, y: -0.5});
			matter.Body.applyForce(this.shrapnelArray[5].body, this.shrapnelArray[5].body.position, {x: 0.5, y: 0});
			
			
			//setTimeout(() =>{console.log("waited before returning")}, 5000);
			
			return this.wasLaunched && (this.bird.didStop() || this.bird.didGoOffScreen()) && this.rightBird == null && this.leftBird == null && !this.bird.exploded;
		}
		else{
			//console.log("RED BIRD REMOVING");
			//console.log("RED DEAD");
			let blackBirdLaunched = false;
			if(this.bird instanceof BlackBird && this.bird.isLaunched){
				blackBirdLaunched = true;
			}
			if(this.bird instanceof BlackBird && this.bird.isLaunched && (this.bird.didGoOffScreen())){
				blackBirdLaunched = false;
			}
			return this.wasLaunched && (this.bird.didStop() || this.bird.didGoOffScreen()) && this.rightBird == null && this.leftBird == null && !blackBirdLaunched;
		}
	...
	unload() {
		...
		this.bird.update();
		if(this.bird instanceof BlackBird && !this.bird.exploded && this.shrapnelArray != []){
			setTimeout(()=>{this.shrapnelArray =[]}, 3000)
		}
		this.bird = null;
