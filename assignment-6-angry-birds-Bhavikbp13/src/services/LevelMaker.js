...
export default class LevelMaker {
	...
	static levelOne() {
		...
		const birdQueue = new BirdQueue([BirdType.Black, BirdType.Red]);
	...
	static levelTwo() {
		...
		const birdQueue = new BirdQueue([BirdType.Red, BirdType.Blue, BirdType.Black, BirdType.Blue]);
	...
	static levelThree() {
		const blocks = [
			...
			new GlassBlock(LevelMaker.START_X + GlassBlock.SPRITE_MEASUREMENTS[Size.Medium].width * 8, CANVAS_HEIGHT - Ground.GRASS.height - GlassBlock.SPRITE_MEASUREMENTS[Size.Medium].height, Size.Large, GlassBlock.ANGLE_LEFT_DIAGONAL),
		];
		...
		const birdQueue = new BirdQueue([BirdType.Red, BirdType.Blue, BirdType.Black, BirdType.Blue, BirdType.Red, BirdType.Black]);
