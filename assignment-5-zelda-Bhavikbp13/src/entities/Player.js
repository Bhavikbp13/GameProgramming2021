...
import PlayerLiftingPot from "../states/entity/player/PlayerLiftingPotState.js";
import PlayerLiftingPotState from "../states/entity/player/PlayerLiftingPotState.js";
import PlayerIdleCarryingState from "../states/entity/player/PlayerIdleCarryingState.js";
import PlayerWalkingCarryingState from "../states/entity/player/PlayerWalkingCarryingState.js";
import PlayerThrowPotState from "../states/entity/player/PlayerThrowPotState.js";
...
export default class Player extends GameEntity {
	...
	constructor() {
		...
		this.liftingPot = Sprite.generateSpritesFromSpriteSheet(
			images.get(ImageName.PlayerLift),
			Player.LIFTING_SPRITE_WIDTH,
			Player.LIFTING_SPRITE_HEIGHT
		);
		this.carryingPot = Sprite.generateSpritesFromSpriteSheet(
			images.get(ImageName.PlayerIdleCarry),
			Player.WALKING_SPRITE_WIDTH,
			Player.WALKING_SPRITE_HEIGHT
		);
		...
		this.liftHitbox = new Hitbox(0, 0, 0, 0, 'green');
	...
	render() {
		...
		if (DEBUG) {
			...
			this.liftHitbox.render(context);
		}
	}
	...
	initializeStateMachine() {
		...
		stateMachine.add(PlayerStateName.LiftingPot, new PlayerLiftingPotState(this));
		stateMachine.add(PlayerStateName.IdleCarrying, new PlayerIdleCarryingState(this));
		stateMachine.add(PlayerStateName.WalkingCarrying, new PlayerWalkingCarryingState(this));
		stateMachine.add(PlayerStateName.ThrowingPot, new PlayerThrowPotState(this));
	...
	receiveHealth(healthAdd){
		if(this.health + healthAdd > Player.MAX_HEALTH){
			this.health = Player.MAX_HEALTH;
		}
		else{
			this.health += healthAdd;
		}
		//console.log("Player health up")
	}
